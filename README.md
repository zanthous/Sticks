...
Approach Rate is a persistent player preference rather than beatmap difficulty. Set it under **Settings → Rulesets → Sticks**, or use lazer's standard decrease/increase scroll-speed bindings (F3/F4 by default) during gameplay. The default is AR 8 / 750 ms, and the selected value is reused across maps without requiring a mod.
